...
def getXlData():
    ...
    produce_list={}
    col_value = []
    wb = open_workbook('testdata.xlsx')
...
def getCrops():
    ...
    for item in produce_list:
        ...
        if b"www.myfitnesspal.com" in link:
            ...
            if b"..." in link:
                if b"food" in link:
                    link = link.replace(b"...", b"categories")
                    itemRequest = rq.get("http://" + link.decode('utf-8'))
                    itemContent = itemRequest.content
                    
                    
                    if b"Member Login" in itemContent:
                        link = link.replace(b"categories", b"calories") 
